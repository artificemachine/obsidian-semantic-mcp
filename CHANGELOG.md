...
- 2026-07-21: feat(release): wire the native (non-Docker) branch of `osm migrate --embedding-dim`, closing the last gap noted when `cmd_migrate` was registered in COMMANDS (PR #31). `_run_migration_snippet()` now detects Docker vs native via `_migrate_target_is_docker()` (checks for a running mcp-server container) and runs the migration through `uv run` locally against `PROJECT_ROOT/src` when native, instead of `docker compose exec`; `DATABASE_URL` falls back to the fixed native default (`postgresql://localhost/obsidian_brain`) since `mode_native_macos` never writes a `.env`. Adds 8 tests (TestMigrateNativeBranch in tests/test_osm_commands.py) and updates the stale withheld-pending-native-path note in tests/test_dimension_migration.py. Also deletes 9 merged branches from origin (chore/job-ready-polish, docs/agents-md-no-version, docs/job-ready-final, docs/recruiter-polish, docs/runbook-ollama-profile-fix, feat/osm-skip-pi, feat/security-correctness, fix/embed-concurrency-cpu-only-thrash, fix/test-launcher-stdin-isolation) per the 2026-07-21 portfolio-ready audit cleanup plan; the other 8 branches it flagged are open Dependabot PRs, left alone. Bump to 0.16.0.
- 2026-07-21: fix(osm-init): native install (`osm init --mode 1`) registered a Claude Desktop/Code MCP entry that could never launch. `_native_entry(vault, db_url)` accepted both params and silently discarded them, returning `env: {}`; `mode_native_macos()` never writes a `.env`, so `src/launcher.py`'s `_validate_env()` found `OBSIDIAN_VAULT`/`DATABASE_URL` set nowhere and exited immediately on every launch attempt. Found by this session's `/arch-audit` run (Stage 6 of the 2026-07-21 portfolio-ready re-audit). `_native_entry()` now populates `env` from its own parameters, using `OBSIDIAN_VAULTS` (comma-separated) for multi-vault installs and `OBSIDIAN_VAULT` for the single-vault case, matching `server.py`'s own `_parse_vault_paths()` convention. Adds a regression test (`tests/test_launcher.py::test_native_entry_env_is_launchable`) that feeds the registered entry's env into `launcher._validate_env()` as the *only* environment, exactly reproducing how an MCP client launches a configured command — this is the test that was missing and let the bug ship undetected through PR #31's dispatch-registration hardening and every prior fresh-clone audit. Also fixes `tests/test_osm_commands.py::test_native_entry_env_vars`, which had pinned the empty-env bug as expected behavior.
- 2026-07-21: chore(dashboard): add structured logging. `src/dashboard.py` (972 lines, bearer-token auth + mutating endpoints `/api/reindex`, `/api/reindex/full`, `/api/prune`, `/api/ollama/start`) had zero `log.*` calls — `log_message()` explicitly suppresses even the default HTTP request-line logging. Found by this session's `/arch-audit` (Stage 6, HIGH). Adds `log = logging.getLogger(__name__)` (reuses `server.py`'s `logging.basicConfig()`, which importing `server` already triggers) and logs: unauthorized requests (client IP + method + path, security-relevant), search failures, reindex start/busy/finish/failure, prune results, ollama-start failures. Incidentally also catches and logs the reindex background thread's previously-unhandled exception path (was a bare try/finally, an exception would silently kill the thread with only a raw Python traceback to stderr). Adds `test_unauthorized_request_is_logged` to `tests/test_dashboard_security.py`.
- 2026-07-21: docs(readme): add Troubleshoot section (top-3 first-install failures + link to docs/RUNBOOK.md) and a Native Install (macOS) section (fixes the README's own broken `#native-install-macos` anchor link, which pointed at a heading that never existed). Fixes `CONTRIBUTING.md`'s stale claims: "All 183 unit tests must pass" (now points to the README test badge instead of a number that will drift again) and "Python 3.11+" (pyproject.toml/README badge both say 3.10+). Fixes `docs/README.md`'s Audits section, which described the folder as holding only `<date>-job-ready.md` files though it now also contains `production-ready`/`portfolio-ready` naming. All 4 findings from Stage 1/3 of the 2026-07-21 portfolio-ready re-audit.
- 2026-07-21: chore(deps): sync `uv.lock` — the 3 Dependabot PRs merged this session (#13 psycopg2-binary, #19 requests, #20 pyyaml) only touched `pyproject.toml`'s version specifiers, never regenerated `uv.lock`, leaving it silently out of sync (`uv lock --check` failed on `main`). CI's `uv sync` resolves fresh at build time so this never broke a build, but a committed stale lockfile defeats reproducible installs for anyone using `uv sync --frozen` or auditing the exact pinned tree. `uv lock` regenerates it to match.
- 2026-07-22: docs(portfolio): stop the README test-count claim from drifting (was "446 tests"; now "400+ tests, see the Tests badge" plus a qualitative without-DB / pg-marker split) and add a "Status: historical/speculative, not current-state" banner to the three undated design docs (PLAN-portable-mcp-config, PLAN-security-correctness, proposed-rename-mnemosyne) so a reviewer landing on the raw file cannot mistake them for the live roadmap. Findings from the 2026-07-22 portfolio-ready audit Stage 3/8 (test-count drift) and Stage 3 (stale planning docs). No code change.
- 2026-07-22: docs(audit): record the 2026-07-22 full portfolio-ready re-audit (all 9 stages, Opus 4.8, none condensed) — verdict NEEDS POLISH, 0 blockers, all 6 hard gates pass; capped only by the cosmetic README test-count drift since fixed in #39. Also records the two 2026-07-21 audit reports (portfolio-ready + arch-audit) that were left untracked at the time.
- 2026-07-22: fix(coordination): route the first-boot full index through reindex_lock(). background_init() called index_vault() directly, bypassing the advisory lock that the reindex tool and dashboard /api/reindex acquire — so an operator re-index firing during a long first-boot index (large vault, CPU-only Ollama) could run a second concurrent index_vault against the shared DB. Non-corrupting (idempotent ON CONFLICT upsert + hash-skip) but wasteful (2x embedding CPU) and a hole in the advertised re-index mutual-exclusion invariant. Boot now indexes under the lock; if held, it skips its own index (the holder is already doing it) and still starts the watcher. Found by the 2026-07-22 portfolio-ready Stage 6 arch-audit (server.py:1185). Adds 2 tests (TestBackgroundInit lock coupling) to tests/test_advisory_lock.py.
- 2026-07-22: chore(release): bump pyproject.toml version 0.16.0 -> 0.16.1 for the boot-index/reindex-lock coordination fix (#41), which had landed on main since v0.16.0 was tagged. uv.lock regenerated to match.
- 2026-07-22: docs(golive): close the 2 findings from the same-day /golive re-audit. docs/README.md audits-folder description now mentions <date>-golive.md (was still describing the pre-rename job-ready/production-ready/portfolio-ready naming only). README.md + CONTRIBUTING.md test-count wording no longer claims the Tests badge shows a count (it only shows pass/fail) — points to CI logs instead. No code change.
- 2026-07-22: fix(coordination): record watcher-detected pgvector column-dimension-mismatch write failures into index_state (dashboard-visible), not just a log.warning. The boot-time dimension_mismatch check in init_db() only runs once at startup; an edit made afterward that fails with the same "expected N dimensions, not M" error was previously silent beyond a per-file log line. Adds _record_watcher_dimension_failure() (read-merge-write against failed_paths, never clobbers an unrelated idle/indexing status) and wires it into VaultEventHandler._handle_upsert, matched narrowly against the specific column-mismatch error phrasing so it does not misfire on the unrelated "vector must have at least 1 dimension" (empty-embedding) DataException. 2 new tests. Found by the 2026-07-22 golive Stage 6 arch-audit.
- 2026-07-22: test(coordination): add a multi-vault regression test for the #41 boot-index/reindex_lock fix — proves reindex_lock() is acquired independently per vault (not hoisted outside the per-vault loop) across a 3-vault list with a free/held/free lock sequence, and that the watcher still starts for every vault regardless of that vaults lock outcome. Closes the "no multi-vault test" gap noted in the 2026-07-22 golive Stage 6 re-audit.
- 2026-07-22: test(perf): add tests/test_unit.py::TestIndexVaultPerf, a stress/perf test for index_vault() bulk-indexing 300 notes with embed()/DB calls mocked out — isolates and times the file-walk/hash/link-index-build orchestration, asserting no scaling blowup. Registers a new "perf" pytest marker. Closes the zero-stress-coverage gap the 2026-07-22 golive Stage 4 test-gate pass flagged as this repos one genuine test-completeness finding.
- 2026-07-22: chore(docker): pin ghcr.io/astral-sh/uv (Dockerfile + Dockerfile.dashboard COPY --from=) and ollama/ollama (docker-compose.yml, both service definitions) to a digest instead of the mutable :latest tag, for reproducible builds. Adds a docker package-ecosystem entry to .github/dependabot.yml so the pins get bumped by PR rather than going stale-by-design, same rationale already documented there for the SHA-pinned GitHub Actions. Closes the 2 LOW findings carried since the 2026-07-21/22 gauntlet passes.
- 2026-07-22: feat(dashboard): restyle the monitoring dashboard with the Cobalt Grid design language (warm-neutral paper + navy-blue ink palette, permanent graph-paper grid background, Newsreader serif headings, Hanken Grotesk body/labels, DM Mono for numeric values). Style-only change: all element IDs, JS logic, fetch/auth behavior, and HTML text content are byte-identical to before. Semantic status colors (green/red/yellow dots, red danger button) intentionally kept for accessibility rather than forced into a strict two-color scheme. Design reference saved to ../semblar/design_references/max_graphic_design_mood/cobalt_grid_01.{html,json}. 55 dashboard-specific tests + full suite (437 passed) unaffected.
- 2026-07-22: fix(dashboard): remove the permanent graph-paper grid background added in the Cobalt Grid restyle (same PR, pre-merge follow-up) — kept the warm-neutral/navy-blue palette and fonts, dropped the grid overlay per feedback that it read as visual noise on a data-dense monitoring page.
- 2026-07-22: fix(dashboard): remove the dark-navy borders on .card/.status/.recent/.search-panel from the Cobalt Grid restyle (same PR, pre-merge follow-up) — flat paper-2 backgrounds instead, palette/fonts unchanged.
- 2026-07-22: fix(dashboard): shift the Cobalt Grid dashboard palette from warm-neutral (cream) to a cool blue-grey paper (#E4E9F2/#D3DCEC), deepened ink to #1F3868 for contrast (same PR, pre-merge follow-up) — per feedback that the beige tone should be more blueish. Fonts/layout unchanged.
- 2026-07-22: chore(release): bump pyproject.toml version 0.16.1 -> 0.16.2, packaging the 3 unreleased commits since v0.16.1: docs golive-naming fixes (#43), the dimension-mismatch/perf/docker-pin polish batch (#44), and the Cobalt Grid dashboard theme (#46). uv.lock regenerated to match.
- 2026-07-22: docs(readme): add a dashboard screenshot above the fold (images/osm-dashboard-ui.png, resized 5084x2572 -> 1600x809 / 289KB), closing the last open finding from the 2026-07-22 golive audit (Stage 1: no visual demo). Adds .allow-binary-paths (images/**) so the pre-commit binary guard permits future README screenshots without a one-shot override.
- 2026-07-22: feat(dashboard): make the "Recently Indexed" panel collapsible — clicking its heading toggles the list via a new toggleRecent() JS function and a rotating arrow indicator, no new dependency. 55 dashboard tests still green.
- 2026-07-22: docs(readme): replace the dashboard screenshot with a sanitized retake — the original exposed real personal vault content (crypto research notes, a third-party name in a filename) in the "Recently Indexed" panel, flagged as a MED finding by the 2026-07-22 golive audit. New screenshot shows self-referential search results (Obsidian setup/naming-convention notes about the tool itself) and demonstrates the new collapsible Recently Indexed panel. Resized 5102x2456 -> 1600x770 (203KB).
- 2026-07-22: fix(coordination): serialize _record_watcher_dimension_failure with an in-process threading.Lock, closing the read-merge-write race the 2026-07-22 golive arch-audit found — two watcher threads recording a dimension-mismatch failure for the same vault at the same time could lose one path via a last-write-wins overwrite. TDD: test_concurrent_dimension_failures_do_not_lose_a_path forces the interleave deterministically via a threading.Event handshake, confirmed RED (lost /b.md) before the fix.
...
- 2026-08-20: fix(mcp): migrate to mcp 2.0.0 decorator-to-callback API and fix a stdin JSON-RPC parse regression it introduced. server.py list_tools/call_tool are now wired via Server(on_list_tools=, on_call_tool=) adapters instead of @server.list_tools()/@server.call_tool() decorators (removed in 2.0.0). Also found and fixed a second, independent break: JSONRPCMessage is now a bare Union type alias with no .model_validate_json(), so every incoming stdin line was silently failing to parse and the server never responded to initialize — switched to the jsonrpc_message_adapter TypeAdapter mcp's own stdio transport uses. TDD: reproduced the initialize hang directly against Server.run() with in-memory streams before fixing. Full suite: 439 passed, 0 failed, 31 skipped (same as pre-migration baseline).
- 2026-08-20: chore(release): bump pyproject.toml version 0.16.4 -> 0.16.5, packaging the mcp 2.0.0 migration from #56 (constructor-callback API adapters + JSONRPCMessage parse fix). uv.lock regenerated to match.
- 2026-09-21: fix(compose): raise mcp-server memory limit 512m -> 2g. Every MCP client session runs its own server.py via docker compose exec inside the one shared container, so concurrent sessions hit the cgroup limit and the OOM killer SIGKILLed a session's server (exit 137), dropping its MCP connection. Verified OOMKilled=true, then OOMKilled=false and healthy after recreate. No version bump.
